...
func VideoLikedCount(videoId int64) (int64, error) {
	var count int64
	// Find likes count in database
	err := Db.Model(Like{}).Where(map[string]interface{}{"video_id": videoId, "liked": 1}).Count(&count).Error
	if err != nil {
		log.Println("LikeDao-Count: return count failed") // Return error message
		return -1, errors.New("find likes count failed")
	}
	log.Println("LikeDao-Count: return count success") // Return correct message
	return count, nil
}

// Get User Id List Who Liked This Video and the count of it
func UsersOfLikeVideo(videoId int64) ([]int64, int64, error) {
	...
	return userIdList, likedCnt, nil
}

// update likes info by user id and video id with liked
...
// func IsVideoLikedByUser(userId int64, videoId int64) (bool, error) {
// 	var isLiked int8
// 	result := Db.Model(Like{}).Select("liked").Where("user_id= ? and video_id= ?", userId, videoId).First(&isLiked)
// 	c := result.RowsAffected
// 	if c == 0 {
// 		return false, errors.New("current user haven not liked current video")
// 	}
// 	if result.Error != nil {
